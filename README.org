...
* Motivation
While [[https://github.com/doomemacs][Doom Emacs]] is an amazing distribution, it is a bit complex and does have a few dependencies. However Doom Emacs's =map!= macro is very simple and intuitive to use, and keybinding in vanilla [[https://www.gnu.org/software/emacs/][GNU Emacs]] (or even with [[https://www.emacswiki.org/emacs/Evil][evil-mode]]) is for me one of its serious downsides, and keybindingss are usually the first thing one wants to customize, so I decided to distill out the =map!= Macro (and the other macros connected to keybinding) into its own package that can be used easily, ergo, 'ez-map'.
